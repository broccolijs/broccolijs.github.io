...
export class MarkdownHandlebars extends Plugin {
  /**
   *
   * @param {Node} markdownNode The source directory for the markdown files
   * @param {Node} templatesNode The source directory for the template files
   ...
   *    },
   *    data: {}, // Data hash passed to templates as `site`
   *    callbacks: {
  ...
  constructor(markdownNode, templatesNode, options) {
    ...
    this.md = Markdown(options.markdownOptions)
      .use(mdHighlight, options.markdownHilightOptions)
      .use(mdAnchor, options.markdownAnchorOptions)
      .use(mdToc);
...
export default function markdownHandlebars(...params) {
  return new MarkdownHandlebars(...params);
};
